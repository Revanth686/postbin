//query photos from firestore db and render them
import React, { useContext } from "react";
import Layout from "@/components/Layout";
import { userAuthContext } from "@/context/userAuth";
import { getPostByUserId } from "@/repository/post.service";
import { DocumentResponse, Post } from "@/shared.types";
...
const Photos: React.FunctionComponent<IPhotosProps> = () => {
  ...
  const renderPost = () => {
    return data.map((item) => {
      console.log(`rendering photos ${JSON.stringify(item)}`);
      if (item.photos.length === 0) return <p>empty...</p>;
      return (
